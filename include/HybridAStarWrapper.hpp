...
geometry_msgs::PoseStamped HybridAStarWrapper::chooseGoal(mappi::objects::Path& goals)
{
    ...
    // To-Do: add goal orientation
    chosen_goal.pose.orientation.w = 1.0;
                                    ...
                                    mappi::objects::Path& out_plan)
{
    ...
    HybridAStar::Node3D* nSolution = HybridAStar::Algorithm::hybridAStar(nStart, nGoal, nodes3D, nodes2D, 
...
void HybridAStarWrapper::fromAStarToMappi(const std::vector<HybridAStar::Node3D>& in_path, mappi::objects::Path& out_path)
{
    ...
    for(int i=0; i < in_path.size(); i++){
        // out_path.x(i)    = in_path[i].getX() * costmap_->getResolution();
        // out_path.y(i)    = in_path[i].getY() * costmap_->getResolution();
        out_path.x(i)    = in_path[i].getX() * HybridAStar::Constants::cellSize;
        out_path.y(i)    = in_path[i].getY() * HybridAStar::Constants::cellSize;
        out_path.yaw(i)  = 0.0;
